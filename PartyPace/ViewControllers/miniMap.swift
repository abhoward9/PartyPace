...
//                }
        group.notify(queue: .main) {
                    ...
                    if let data = data, let dataString = String(data: data, encoding: .utf8) {
                        ...
                        if let jsonData = dataString.data(using: .utf8) {
                            ...
                            do {
                                ...
                                routeID = latestUserRoute.results[0].id
            ...
            group2.notify(queue: .main) {
                self.requestRouteDetailsAndDisplayRoute(identifier: routeID)
...
//                            let mapRect = MKMapRect(x: boundingBox[0].lng, y: boundingBox[0].lat, width: width, height: height)
